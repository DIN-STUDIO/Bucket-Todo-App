import UIKit
import SnapKit
import Alamofire

class TodoVC: UIViewController {
    
    var bucketContent: String? // bucket의 content를 받을 프로퍼티
    let bucketLabel = UILabel() // bucket content를 표시할 레이블
    let addTodoDetailBtn = UIButton() // 상세 투두 추가하기 버튼
    var tableView = UITableView()
    var todoList = [Todo]() // 투두 리스트 데이터를 저장할 배열
    var bucketId: Int? // bucketId를 받을 프로퍼티
    var blurEffectView: UIVisualEffectView? // 블러 효과 뷰를 추적하기 위한 프로퍼티
    
    override func viewDidLoad() {
        super.viewDidLoad()
        
        self.view.backgroundColor = .white
        
        // 상단 뷰 설정
        let topView = UIView()
        topView.backgroundColor = UIColor(red: 246/255.0, green: 246/255.0, blue: 246/255.0, alpha: 1.0) // 상단 뷰 배경 색 설정
        topView.layer.cornerRadius = 50
        topView.layer.maskedCorners = [.layerMaxXMinYCorner, .layerMinXMinYCorner] // 아래쪽 모서리만 라운딩
        
        self.view.addSubview(topView)
        
        topView.snp.makeConstraints { make in
            make.top.equalTo(self.view)
            make.leading.trailing.equalTo(self.view)
            make.height.equalTo(200)
        }
        
        // bucketLabel 설정
        bucketLabel.textAlignment = .center
        bucketLabel.font = UIFont.systemFont(ofSize: 24, weight: .bold)
        bucketLabel.textColor = .black
        
        topView.addSubview(bucketLabel)
        
        bucketLabel.snp.makeConstraints {
            $0.centerX.equalTo(topView)
            $0.top.equalTo(topView).offset(120) // 위로 20 이동
        }
        
        if let content = bucketContent {
            bucketLabel.text = content
        }
        
        // 상세 투두 추가하기 버튼 설정
        addTodoDetailBtn.setTitle("상세 투두 추가하기", for: .normal)
        addTodoDetailBtn.setTitleColor(.white, for: .normal)
        addTodoDetailBtn.backgroundColor = UIColor(red: 255/255.0, green: 157/255.0, blue: 0/255.0, alpha: 1.0)
        addTodoDetailBtn.layer.cornerRadius = 10
        addTodoDetailBtn.titleLabel?.font = UIFont.systemFont(ofSize: 14, weight: .bold) // 폰트 크기 줄이기
        addTodoDetailBtn.addTarget(self, action: #selector(addTodoDetailBtnTapped), for: .touchUpInside)
        
        self.view.addSubview(addTodoDetailBtn)
        
        addTodoDetailBtn.snp.makeConstraints {
            $0.trailing.equalTo(self.view.snp.trailing).offset(-20)
            $0.bottom.equalTo(self.view.safeAreaLayoutGuide.snp.bottom).offset(-20)
            $0.width.equalTo(184)
            $0.height.equalTo(20) // 높이 줄이기
        }
        
        // 테이블뷰 설정
        tableView.dataSource = self
        tableView.delegate = self
        tableView.separatorStyle = .none
        tableView.register(TodoCell.self, forCellReuseIdentifier: "TodoCell")
        
        self.view.addSubview(tableView)
        
        tableView.snp.makeConstraints {
            $0.top.equalTo(topView.snp.bottom).offset(20)
            $0.leading.trailing.equalTo(self.view).inset(10) // padding 추가
            $0.bottom.equalTo(addTodoDetailBtn.snp.top).offset(-20)
        }
        
        // 투두 리스트 불러오기
        fetchTodoList()
    }
    
    @objc func addTodoDetailBtnTapped() {
        let addVC = AddTodoModal()
        addVC.modalPresentationStyle = .overCurrentContext
        addVC.view.backgroundColor = UIColor(white: 1.0, alpha: 0.5) // 흰색에 투명도 적용
        addVC.modalTransitionStyle = .crossDissolve
        addVC.bucketId = bucketId
        addVC.onDismiss = { [weak self] in
            // 블러 효과 제거
            self?.blurEffectView?.removeFromSuperview()
        }
        addVC.onSubmit = { [weak self] text in
            // 텍스트를 받아와서 배열에 추가하고 테이블뷰 갱신
            self?.fetchTodoList()
        }
        self.present(addVC, animated: true) {
            // 블러 처리
            let blurEffect = UIBlurEffect(style: .light)
            let blurEffectView = UIVisualEffectView(effect: blurEffect)
            blurEffectView.frame = self.view.bounds
            blurEffectView.autoresizingMask = [.flexibleWidth, .flexibleHeight]
            self.view.addSubview(blurEffectView)
            self.view.bringSubviewToFront(addVC.view)
            self.blurEffectView = blurEffectView // 블러 효과 뷰를 저장
            
            // 제약 조건 설정은 뷰가 추가된 후에 해야 합니다.
            addVC.view.snp.makeConstraints { make in
                make.center.equalTo(self.view)
                make.width.equalTo(self.view).multipliedBy(0.8)
                make.height.equalTo(self.view).multipliedBy(0.6)
            }
        }
    }
    
    func fetchTodoList() {
        guard let bucketId = bucketId else {
            print("Bucket ID가 없습니다.")
            return
        }
        
        guard let jwtToken = UserDefaults.standard.string(forKey: "jwtToken") else {
            print("JWT 토큰이 없습니다.")
            return
        }
        
        let headers: HTTPHeaders = [
            "Authorization": "Bearer \(jwtToken)"
        ]
        
        let parameters: [String: Any] = [
            "bucketId": bucketId
        ]
        
        AF.request("http://na2ru2.me:5152/todos", method: .get, parameters: parameters, headers: headers)
            .validate(statusCode: 200..<300)
            .responseDecodable(of: TodoResponse.self) { response in
                switch response.result {
                case .success(let todoResponse):
                    self.todoList = todoResponse.response
                    DispatchQueue.main.async {
                        self.tableView.reloadData()
                    }
                    
                    // 투두 리스트 출력
                    for todo in self.todoList {
                        print("Todo ID: \(todo.todoId), Content: \(todo.content)")
                    }
                    
                case .failure(let error):
                    print("Error fetching todo list: \(error)")
                }
            }
    }
}

// 테이블뷰 데이터소스 및 델리게이트 구현
extension TodoVC: UITableViewDataSource, UITableViewDelegate {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return todoList.count
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "TodoCell", for: indexPath) as! TodoCell
        let todo = todoList[indexPath.row]
        cell.configure(content: todo.content)
        return cell
    }
    
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        return 100 // 셀의 높이를 증가시켜 여백을 포함
    }
    
    func tableView(_ tableView: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        // 셀의 contentView에 하단 여백 추가
        let bottomSpace: CGFloat = 20
        let maskLayer = CALayer()
        maskLayer.frame = CGRect(x: 0, y: cell.contentView.frame.height - bottomSpace, width: cell.contentView.frame.width, height: bottomSpace)
        maskLayer.backgroundColor = UIColor.white.cgColor
        cell.contentView.layer.addSublayer(maskLayer)
    }
}

// 커스텀 셀 구현
class TodoCell: UITableViewCell {
    
    let titleLabel = UILabel()
    
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        super.init(style: style, reuseIdentifier: reuseIdentifier)
        
        self.layer.cornerRadius = 10
        self.clipsToBounds = true
        
        // 타이틀 라벨 설정
        titleLabel.font = UIFont.systemFont(ofSize: 16, weight: .bold)
        titleLabel.textColor = .black
        
        contentView.backgroundColor = UIColor(red: 246/255.0, green: 246/255.0, blue: 246/255.0, alpha: 1.0) // 셀 배경 색 설정
        
        // 뷰에 추가
        contentView.addSubview(titleLabel)
        
        // 레이아웃 설정
        titleLabel.snp.makeConstraints {
            $0.leading.equalTo(contentView).offset(10)
            $0.top.equalTo(contentView).offset(10)
        }
    }
    
    required init?(coder: NSCoder) {
        fatalError("init(coder:) has not been implemented")
    }
    
    func configure(content: String) {
        titleLabel.text = content
    }
}

